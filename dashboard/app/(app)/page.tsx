...
// Client-selectable auto-destroy windows (minutes). Default is 20.
const TIMEOUT_PRESETS = [5, 10, 15, 20] as const
...
export default function BrowsersPage() {
  ...
  const [timeoutMin, setTimeoutMin] = useState<number>(20)
  ...
  async function handleCreate() {
    ...
    setError(null)
    try {
      const b = await createBrowser("cloak", timeoutMin * 60)
      setBrowsers((p) => [b, ...p])
      setView(b)
      setName("")
    } catch (e: any) {
    ...
    <>
      <header className="flex items-center gap-3 border-b-2 border-border px-4 py-2.5">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          <div className="hidden items-center gap-1 rounded-base border-2 border-border bg-secondary-background p-0.5 sm:flex">
            {TIMEOUT_PRESETS.map((m) => (
              <button
                key={m}
                onClick={() => setTimeoutMin(m)}
                className={`rounded-base px-2 py-1 text-xs font-bold ${
                  timeoutMin === m ? "bg-main text-main-foreground" : "text-muted-foreground hover:text-foreground"
                }`}
                title={`Auto-destroy after ${m} minutes`}
              >
                {m}m
              </button>
            ))}
