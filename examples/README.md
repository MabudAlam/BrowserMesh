...
## `raw_cdp.py`
Uses only `requests` + `websockets`. Creates a browser, prints its CDP and
viewer URLs, navigates to example.com, prints the page title, then deletes the
browser.

## `browser_use_agent.py`
Runs a **browser-use** agent that drives a BrowserMesh browser to visit
`mabud.dev` and `dejan.works` and extract their portfolio info (about,
...
- Browsers take a few seconds to become ready; the demos **poll** status until
  it reports `Running` before driving.
- To watch a run live, open the dashboard (`http://localhost:4003`) and click
  **View**, or use the viewer URL each example prints.
